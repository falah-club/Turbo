...
async fn m() {
    ...
    Server::new(acceptor)
        .serve(Router::with_hoop(auth_handler).goal(authenticate))
        .await;
}

#[handler]
async fn authenticate(req: &mut Request, depot: &mut Depot, res: &mut Response) -> anyhow::Result<()> {
